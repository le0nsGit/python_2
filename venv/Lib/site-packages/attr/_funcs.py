...
def asdict(
    ...
    Args:
        ...
        filter (callable): A callable whose return code determines whether an
        ...
        dict_factory (callable): A callable to produce dictionaries from. For
            example, to produce ordered dictionaries instead of normal Python
            dictionaries, pass in ``collections.OrderedDict``.
        retain_collection_types (bool): Do not convert to ``list`` when
            encountering an attribute whose type is ``tuple`` or ``set``. Only
            meaningful if ``recurse`` is ``True``.
        value_serializer (Optional[callable]): A hook that is called for every
            attribute or dict key/value. It receives the current instance, field
            and value and must return the (updated) value. The hook is run
...
def astuple(
    ...
    Args:
        ...
        filter (callable): A callable whose return code determines whether an
            ...
            Is called with the `attr.Attribute` as the first argument and the
            value as the second argument.
        tuple_factory (callable): A callable to produce tuples from. For
            example, to produce lists instead of tuples.
        retain_collection_types (bool): Do not convert to ``list`` or ``dict``
            when encountering an attribute which type is ``tuple``, ``dict`` or
            ``set``. Only meaningful if ``recurse`` is ``True``.
...
def assoc(inst, **changes):
    ...
    Raises:
        attr.exceptions.AttrsAttributeNotFoundError: If *attr_name* couldn't be
            found on *cls*.
        attr.exceptions.NotAnAttrsClassError: If *cls* is not an ``attrs``
            class.
    """
...
def resolve_types(cls, globalns=None, localns=None):
    ...
    Returns:
        *cls* so you can use this function also as a class decorator. Please
        note that you have to apply it **after** `attr.s` . That means the
        decorator has to come in the line **before** `attr.s` .
