...
def hash_secret(
    ...
    Args:
        ...
        salt (bytes): A salt_. Should be random and different for each secret.
        time_cost:
        memory_cost:
        parallelism:
        hash_len:
...
def verify_secret(hash, secret, type):
    ...
    .. versionchanged:: 16.1.0
       Raise :exc:`~argon2.exceptions.VerifyMismatchError` on mismatches instead
       of its more generic superclass.

    Args:
        hash (bytes): An encoded Argon2 hash as returned by :func:`hash_secret`.
        secret (bytes): The secret to verify whether it matches the one in
            *hash*.
    ...
    Raises:
        argon2.exceptions.VerifyMismatchError: If verification fails because
            *hash* is not valid for *secret* of *type*.
...
def core(context, type):
    ...
    Args:
        context: A CFFI Argon2 context object (i.e. an ``struct
Argon2_Context``
            / ``argon2_context``).
        type (int): Which Argon2 variant to use. You can use the ``value`` field
            of :class:`Type`'s fields.
    ...
    Warning:
        ...
        (e.g. clearing buffers). The structure of the *context* object can, has,
        and will change with *any* release!
...
def error_to_str(error):
    ...
    Args:
        error (int): An Argon2 error code as returned by :func:`core`.
