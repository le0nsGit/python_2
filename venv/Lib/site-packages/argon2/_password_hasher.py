...
class PasswordHasher(object):
    ...
    Args:
        ...
        memory_cost (int): Defines the memory usage, given in kibibytes_.
        parallelism (int): Defines the number of parallel threads (*changes* the
            resulting hash value).
        hash_len (int): Length of the hash in bytes.
        salt_len (int): Length of random salt to be generated for each password.
        encoding (str): The Argon2 C library expects bytes. So if :meth:`hash`
            or :meth:`verify` are passed an unicode string, it will be encoded
            using this encoding.
        type (Type): Argon2 type to use. Only change for interoperability with
            legacy systems.
    """
    ...
    def hash(self, password):
        ...
        Args:
            password (bytes or unicode): Password to hash.
    ...
    def verify(self, hash, password):
        ...
        Args:
            hash (bytes or unicode): An encoded hash as returned from
                :meth:`PasswordHasher.hash`.
            password (bytes or unicode): The password to verify.

        Returns:
            bool: ``True`` on success, raise
            :exc:`~argon2.exceptions.VerificationError` otherwise.

        Raises:
            argon2.exceptions.VerifyMismatchError: If verification fails because
                *hash* is not valid for *password*.
            argon2.exceptions.VerificationError: If verification fails for other
                reasons.
            argon2.exceptions.InvalidHash: If *hash* is so clearly invalid, that
                it couldn't be passed to Argon2.
    ...
    def check_needs_rehash(self, hash):
        ...
        common approach is to do that whenever a user logs in, since that should
        be the only time when you have access to the cleartext password.
