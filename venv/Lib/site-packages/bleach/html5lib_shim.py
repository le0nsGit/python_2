...
class InputStreamWithMemory(object):
    ...
    def get_tag(self):
        ...
        we know that everything from that point to when this method is called is
        the "tag" that is being tokenized.
        """
...
class BleachHTMLParser(HTMLParser):
    ...
    def __init__(self, tags, strip, consume_entities, **kwargs):
        """
        Args:
            tags: list of allowed tags--everything else is either stripped or
                escaped; if None, then this doesn't look at tags at all
            strip: whether to strip disallowed tags (True) or escape them
                (False); if tags=None, then this doesn't have any effect
            consume_entities: whether to consume entities (default behavior) or
                leave them as is when tokenizing (BleachHTMLTokenizer-added
                behavior)
            **kwargs:
        """
...
def convert_entity(value):
    ...
    Args:
        value: the string (minus the ``&`` and ``;`` part) to convert

    Returns:
        unicode character or None if it's an ambiguous ampersand that doesn't
        match a character entity
    """
    if value[0] == "#":
        if len(value) < 2:
            return None
...
def convert_entities(text):
    ...
    Args:
        text: the text to convert entities in
...
def match_entity(stream):
    ...
    ";". This ignoresambiguous character entities that have no ";" at the end.
...
class BleachHTMLSerializer(HTMLSerializer):
    ...
    def serialize(self, treewalker, encoding=None):
        """Wrap HTMLSerializer.serialize and conver & to &amp; in attribute
        values

        Note that this converts & to &amp; in attribute values where the &
        isn't already part of an unambiguous character entity.
