...
class Cleaner(object):
    ...
    This cleaner is a security-focused function whose sole purpose is to
    ...
    def __init__(
        ...
        Args:
            ...
            attributes (dict): allowed attributes; can be a callable, list or
                dict; defaults to ``bleach.sanitizer.ALLOWED_ATTRIBUTES``
            styles (list): allowed list of css styles; defaults to
                ``bleach.sanitizer.ALLOWED_STYLES``
            protocols (list): allowed list of protocols for links; defaults to
            ...
            filters (list): list of html5lib Filter classes to pass streamed
                content through
...
def attribute_filter_factory(attributes):
    ...
    The attributes value can take one of several shapes. This returns a
    filter function appropriate to the attributes value. One nice thing about
    this is that there's less if/then shenanigans in the ``allow_token`` method.
...
class BleachSanitizerFilter(html5lib_shim.SanitizerFilter):
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            attributes (dict): allowed attributes; can be a callable, list or
                dict; defaults to ``bleach.sanitizer.ALLOWED_ATTRIBUTES``
            strip_disallowed_elements (bool): whether or not to strip disallowed
                elements
    ...
    def sanitize_characters(self, token):
        ...
        Since we don't want that, we extract entities here and convert them
        to Entity tokens so the serializer will let them be.

        Args:
            token: the Characters token to work on
    ...
    def sanitize_uri_value(self, value, allowed_protocols):
        ...
        Args:
            value: the uri value to sanitize
