...
def build_email_re(tlds=TLDS):
    ...
    If you want a different set of tlds, pass those in and stomp on the
    existing ``email_re``::
...
class Linker(object):
    ...
    This function converts strings that look like URLs, domain names and
    email addresses in text that may be an HTML fragment to links, while
    preserving:
    ...
    def __init__(
        ...
        Args:
            callbacks (list): list of callbacks to run when adjusting tag
                attributes; defaults to ``bleach.linkifier.DEFAULT_CALLBACKS``
            skip_tags (list): list of tags that you don't want to linkify the
                contents of; for example, you could set this to ``['pre']`` to
                skip linkifying contents of ``pre`` tags
            parse_email (bool): whether or not to linkify email addresses
            url_re (re): url matching regex
            email_re (re): email matching regex
            recognized_tags (list-of-strings): the list of tags that linkify
                knows about; everything else gets escaped
    ...
    def linkify(self, text):
        ...
        Args:
            text (str): the text to add links to

        Returns:
            linkified text as unicode

        Raises:
            TypeError: if ``text`` is not a text type
        """
...
class LinkifyFilter(html5lib_shim.Filter):
    ...
    * edit existing links by running them through callbacks--the default is to
    ...
    def apply_callbacks(self, attrs, is_new):
        ...
        Callbacks can return an adjusted attrs dict or ``None``. In the case
        of ``None``, we stop going through callbacks and return that and the
        link gets dropped.

        Args:
            attrs (dict): map of ``(namespace, name)`` -> ``value``
            is_new (bool): whether or not this link was added by linkify
    ...
    def handle_a_tag(self, token_buffer):
        ...
        This could adjust the link or drop it altogether depending on what
        the callbacks return.
