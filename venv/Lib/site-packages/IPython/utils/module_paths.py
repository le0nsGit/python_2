...
def find_mod(module_name):
    ...
    `module_name` .
        ...
        - If `module_name` refers to a module directory, then return path to
          __init__ file. - If `module_name` is a directory without an
          __init__file, return None.
        - If module is missing or does not have a `.py` or `.pyw` extension,
          return None. - Note that we are not interested in running bytecode.
    ...
    Returns:
        str: **module_path** -- Path to module `module_name` , its __init__.py,
        or None, depending on above conditions.
    """
