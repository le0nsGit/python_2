...
class LSString(str):
    ...
    These are normal strings, but with the special attributes:
        ...
        original value (the string itself). .s (or .spstr): value as
        whitespace-separated string. .p (or .paths): list of path objects
        (requires path.py package)

    Any values which require transformations are computed only once and
    cached.

    Such strings are very useful to efficiently interact with the shell,
    which typically only understands whitespace-separated options for commands.
    """
...
class SList(list):
    ...
    * .p (or .paths): list of path objects (requires path.py package)
    ...
    def grep(self, pattern, prune = False, field = None):
        ...
        This is case-insensitive. If prune is true, return all items NOT
...
def indent(instr,nspaces=4, ntabs=0, flatten=False):
    ...
    Args:
        ...
        flatten (bool (default: False)): Whether to scrub existing indentation.
            If True, all lines will be aligned to the same indentation. If
            False, existing indentation will be strictly increased.

    Returns:
        string indented by ntabs and nspaces.: **str|unicode**
    """
    if instr is None:
        return
    ind = '\t'*ntabs+' '*nspaces
...
def strip_email_quotes(text):
    ...
    Examples:
        ...
        So if any line has no quote marks ('>') , then none are stripped from
        any of them
...
def compute_item_matrix(items, row_first=False, empty=None, *args, **kwargs) :
    ...
    Args:
        ...
        row_first ((default False)): Whether to compute columns for a row-first
            matrix instead of column-first (default).
        empty ((default None)): default value to fill list if needed
        *args:
        **kwargs:

    Returns:
        * *strings_matrix* -- nested list of string, the outer most list
          contains as many list as rows, the innermost lists have each as many
          element as columns. If the total number of elements in `items` does
          not equal the product of rows*columns, the last element of some lists
...
def columnize(items, row_first=False, separator='  ', displaywidth=80, spread=False):
    ...
    Args:
        ...
        row_first ((default False)): Whether to compute columns for a row-first
            matrix instead of column-first (default).
        separator (str, optional [default is two spaces]): The string that
            separates columns.
        displaywidth (int, optional [default is 80]): Width of the display in
