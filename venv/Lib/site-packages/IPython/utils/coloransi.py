...
class InputTermColors:
    ...
    and 002 so that readline can properly know the length of each line and can
    wrap lines accordingly. Use this class for any colored text which needs to
    be used in input prompts, such as in calls to raw_input().
...
class ColorSchemeTable(dict):
    ...
    active_scheme_name -> obvious active_colors -> actual color table of the
    active scheme
    """
    ...
    def __init__(self, scheme_list=None, default_scheme=''):
        ...
        created with a list of valid color schemes AND the specification for the
