...
def read_no_interrupt(p):
    ...
    running in the background, often interrupts are raised that stop the command
    from completing.
...
def process_handler(cmd, callback, stderr=subprocess.PIPE):
    ...
    Args:
        cmd (str or list): A command to be executed by the system, using
            :class:`subprocess.Popen`. If a string is passed, it will be run in
            the system shell. If a list is passed, it will be used directly as
            arguments.
        callback (callable): A one-argument function that will be called with
            the Popen object.
        stderr (file descriptor number, optional): By default this is set to
            ``subprocess.PIPE``, but you can also pass the value
            ``subprocess.STDOUT`` to force the subprocess' stderr to go into the
            same file descriptor as its stdout. This is useful to read stdout
...
def getoutput(cmd):
    ...
    Args:
        cmd (str or list): A command to be executed in the system shell.

    Returns:
        * **output** (*str*) -- A string containing the combination of stdout
          and stderr from the
...
def get_output_error_code(cmd):
    """Return (standard output, standard error, return code) of executing cmd in
    a shell.
...
def arg_split(s, posix=False, strict=True):
    ...
    function, but with a default of posix=False for splitting, so that quotes in
    inputs are respected.

    if strict=False, then any errors shlex.split would raise will result in
    the unparsed remainder being the last element of the list, rather than
    raising. This is because we sometimes use arg_split to parse things other
