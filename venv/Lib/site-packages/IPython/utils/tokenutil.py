...
def line_at_cursor(cell, cursor_pos=0):
    ...
    Used for calling line-based APIs that don't support multi-line input,
    yet.
    ...
    Returns:
        (string, integer): **(line, offset)** -- The line with the current
        cursor, and the character offset of the start of the line.
    """
    offset = 0
...
def token_at_cursor(cell, cursor_pos=0):
    ...
    Function calls are prioritized, so the token for the callable will be
    returned if the cursor is anywhere inside the call.

    Args:
        cell (unicode): A block of Python code
        cursor_pos (int): The location of the cursor in the block where the
