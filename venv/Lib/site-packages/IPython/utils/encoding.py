...
def get_stream_enc(stream, default=None):
    ...
    check for the encoding attribute prior to returning it, and return a default
    if it doesn't exist or evaluates as False. ``default`` is None if not
    provided.
...
def getdefaultencoding(prefer_stream=True):
    ...
    sensible platform default (that respects LANG environment), and finally to
    sys.getdefaultencoding() which is the most conservative option, and usually
    UTF8 as of Python 3.
