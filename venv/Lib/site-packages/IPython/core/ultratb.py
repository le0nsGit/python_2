...
def findsource(object):
    ...
    or code object. The source code is returned as a list of all the lines in
    the file and the line number indexes a line in that list. An IOError is
...
def _format_traceback_lines(lnum, index, lines, Colors, lvals, _line_format):
    ...
    Args:
        ...
        index (int):
        lines (list[string]):
        Colors: ColorScheme used.
        lvals (bytes): Values of local variables, already colored, to inject
            just after the error line.
        _line_format (f (str) -> (str, bool)): return (colorized version of str,
            failure to do so)
    """
...
class ListTB(TBTools):
    ...
    before printing (such as console replacements based on the code module from
    ...
    def structured_traceback(self, etype, evalue, etb=None, tb_offset=None,
        ...
        Args:
            ...
            etb (object): If list: List of frames, see class docstring for
                details. If Traceback: Traceback of the exception.
            tb_offset (int, optional): Number of frames in the traceback to
                skip. If not given, the instance evalue is used (set in
                constructor).
            context (int, optional): Number of lines of context information to
                print.
    ...
    def _format_list(self, extracted_list):
        ...
        contain internal newlines as well, for those items whose source text
        line is not None.
    ...
    def _format_exception_only(self, etype, value):
        ...
        in a newline. Normally, the list contains a single string; however, for
...
class VerboseTB(TBTools):
    ...
    def debugger(self, force=False):
        ...
        Keywords:
            ...
            - force(False): by default, this routine checks the instance
              call_pdb flag and does not actually invoke the debugger if the
              flag is false. The 'force' option forces the debugger to activate
              even if the flag is false.

        If the call_pdb flag is set, the pdb interactive debugger is invoked.
        In all cases, the self.tb reference to the current traceback is deleted
        to prevent lingering references which hamper memory management.

        Note that each call to pdb() does an 'import readline', so if your
        app requires a special setup for the readline completers, you'll have to
...
class FormattedTB(VerboseTB, ListTB):
    ...
    occurs with python programs that themselves execute other python code, like
    Python shells).
    """
