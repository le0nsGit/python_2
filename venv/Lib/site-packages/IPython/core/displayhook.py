...
class DisplayHook(Configurable):
    ...
    def compute_format_data(self, result):
        ...
        In the default implementation the format data is a :class:`dict` of key
        value pair where the keys are valid MIME types and the values are
        JSON'able data structure containing the raw data for that MIME type. It
        is up to frontends to determine pick a MIME to to use and display that
        ...
        Args:
            result (object): The Python object passed to the display hook, whose
                format will be computed.

        Returns:
            dict: **(format_dict, md_dict)** -- format_dict is a :class:`dict`
            whose keys are valid MIME types and values are JSON'able raw data
            for that MIME type. It is recommended that all return values of this
            should always include the "text/plain" MIME type representation of
            the object. md_dict is a :class:`dict` with the same MIME type keys
    ...
    def write_format_data(self, format_dict, md_dict=None) -> None:
        ...
        override this method to send the entire `format_dict` to the frontends.

        Args:
            format_dict (dict): The format dict for the object passed to
                `sys.displayhook` .
            md_dict (dict (optional)): The metadata dict to be associated with
                the display data.
        """
