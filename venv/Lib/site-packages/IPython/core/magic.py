...
def magics_class(cls):
    ...
    ensure that all the methods that have been decorated as line/cell magics get
    correctly registered in the class instance. This is necessary because when
    method decorators run, the class does not exist yet, so they temporarily
    store their information into a module global. Application of this class
    decorator copies that global data to the class instance and clears the
    global.

    Obviously, this mechanism is not thread-safe, which means that the
    *creation* of subclasses of Magic should only be done in a single-thread
    context. Instantiation of the classes has no restrictions. Given that these
    classes are typically created at IPython startup time and before user
...
def no_var_expand(magic_func):
    ...
    before passing the line to the magic function. This is not always desirable,
    e.g. when the magic executes Python code (%timeit, %time, etc.). Decorate
    magics with `@no_var_expand` to opt-out of variable expansion.
...
class MagicsManager(Configurable):
    ...
    def lsmagic_docs(self, brief=False, missing=''):
        ...
        The return dict has the keys 'line' and 'cell', corresponding to the
        two types of magics we support. Each value is a dict keyed by magic name
        whose value is the function docstring. If a docstring is unavailable,
        the value of `missing` is used instead.

        If brief is True, only the first line of each docstring will be
        returned.
    ...
    def register(self, *magic_objects):
        ...
        Take one or more classes or instances of classes that subclass the
        main `core.Magic` class, and register them with IPython to use the magic
        ...
        The provided arguments can be an arbitrary mix of classes and
        instances.
    ...
    def register_function(self, func, magic_kind='line', magic_name=None):
        ...
        standalone function. The functions should have the following signatures:

        * For line magics: `def f(line)`
        * For cell magics: `def f(line, cell)`
        * For a function that does both: `def f(line, cell=None)`

        In the latter case, the function will be called with `cell==None`
        when invoked as `%f` , and with cell as a string when invoked as `%%f` .

        Args:
            func (callable): Function to be registered as a magic.
            magic_kind (str): Kind of magic, one of 'line', 'cell' or
                'line_cell'
            magic_name (optional str): If given, the name the magic will have in
                the IPython namespace. By default, the name of the function
                itself is used.
        """
    ...
    def register_alias(self, alias_name, magic_name, magic_kind='line', magic_params=None):
        ...
        The alias is an instance of :class:`MagicAlias`, which holds the name
        and kind of the magic it should call. Binding is done at call time, so
        if the underlying magic function is changed the alias will call the new
        function.
...
class Magics(Configurable):
    ...
    functions should accept a string, which they can parse for their own needs.
    This can make some functions easier to type, eg `%cd ../` vs. `%cd("../")`
    ...
    def parse_options(self, arg_str, opt_str, *long_opts, **kw):
        ...
        This allows us to easily expand variables, glob files, quote arguments,
        etc.
...
class MagicAlias(object):
    ...
    at call time, so if the underlying magic changes the alias will call the new
    function.
