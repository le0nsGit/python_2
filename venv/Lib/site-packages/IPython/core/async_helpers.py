...
def _pseudo_sync_runner(coro):
    """A runner that does not really allow async execution, and just advance the
    coroutine.
...
class _AsyncSyntaxErrorVisitor(ast.NodeVisitor):
    ...
    the implementation involves wrapping the repl in an async function, it is
    erroneously allowed (e.g. yield or return at the top level)
    """
    def __init__(self):
...
def _async_parse_cell(cell: str) -> ast.AST:
    ...
    It will return an abstract syntax tree parsed as if async and await
    outside of a function were not a syntax error.
...
def _should_be_async(cell: str) -> bool:
    ...
    Attempt to parse the block of code, it it compile we're fine. Otherwise
    we wrap if and try to compile.
