...
def leading_indent(lines):
    ...
    be removed from each following line in the cell.
...
class PromptStripper:
    ...
    Args:
        prompt_re (regular expression): A regular expression matching any input
            prompt (including continuation, e.g. ``...``)
        initial_re (regular expression, optional): A regular expression matching
            only the initial prompt, but not continuation. If no initial
            expression is given, prompt_re will be used everywhere. Used mainly
            for plain Python prompts (``>>>``), where the continuation prompt
...
def _find_assign_op(token_line) -> Union[int, None]:
    """Get the index of the first assignment in the line ('=' not inside
    brackets)
...
def assemble_continued_line(lines, start: Tuple[int, int], end_line: int):
    ...
    Continued lines are lines ending in ``\``, and the line following the
    last ``\`` in the block.
    ...
    This statement contains four continued line pieces. Assembling these
    pieces into a single line would give::
...
class TokenTransformBase:
    ...
    Subclasses need to implement one class method (find) and one regular
    method (transform).

    The priority attribute can select which transformation to apply if
    multiple transformers match in the same place. Lower numbers have higher
    priority. This allows "%magic?" to be turned into a help call rather than a
    magic call.
    """
    ...
    def find(cls, tokens_by_line):
        ...
        Tokens are grouped into logical lines for convenience, so it is easy
        to e.g. look at the first token of each line. *tokens_by_line* is a list
        of lists of tokenize.TokenInfo objects.
    ...
    def transform(self, lines: List[str]):
        ...
        Takes a list of strings representing physical lines, returns a
        similar list of transformed lines.
...
def _make_help_call(target, esc, next_input=None):
    """Prepares a pinfo(2)/psearch call from a target name and the escape (i.e.
    ? or ??)
...
class TransformerManager:
    ...
    The key methods for external use are ``transform_cell()`` and
    ``check_complete()``.
    """
    def __init__(self):
    ...
    def do_one_token_transform(self, lines):
        ...
        Python, not using lots of IPython special syntax, so this shouldn't be a
        performance issue.
    ...
    def check_complete(self, cell: str):
        ...
        Returns:
            * **status** (*str*) -- One of 'complete', 'incomplete', or
              'invalid' if source is not a prefix of valid code.
            * **indent_spaces** (*int or None*) -- The number of spaces by which
              to indent the next line of code. If status is not 'incomplete',
              this is None.
        """
