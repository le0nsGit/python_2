...
class NonAsciiTest(unittest.TestCase):
    ...
    def test_nonascii_path(self):
        ...
        with TemporaryDirectory(suffix=u'Ã©') as td:
            fname = os.path.join(td, u"fooÃ©.py")
            with open(fname, "w") as f:
                f.write(file_1)
    ...
    def test_iso8859_5(self):
        with TemporaryDirectory() as td:
            ...
            with tt.AssertPrints("ZeroDivisionError"):
                with tt.AssertPrints(u'Ð´Ð±Ð˜Ð–', suppress=False):
                    ip.run_cell('fail()')
    
    def test_nonascii_msg(self):
        cell = u"raise Exception('Ã©')"
        expected = u"Exception('Ã©')"
        ...
        with tt.AssertPrints(u"Exception: Ã©"):
            ip.run_cell(cell)
