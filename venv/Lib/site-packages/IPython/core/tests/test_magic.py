...
def test_config():
    """test that config magic does not raise can happen if Configurable init is
    moved too early into Magics.__init__ as then a Config object will be
    registered as a magic.
    """
    ## should not raise.
    _ip.magic('config')

def test_config_available_configs():
    """test that config magic prints available configs in unique and sorted
    order.
    """
...
def doctest_hist_r():
    ...
    XXX - This test is not recording the output correctly. For some reason,
    in testing mode the raw history isn't getting populated. No idea why.
    Disabling the output checking for now, though at least we do run it.

    In [1]: 'hist' in _ip.lsmagic() Out[1]: True

    In [2]: x=1

    In [3]: %hist -rl 2 x=1 # random %hist -r 2
    """
...
def doctest_hist_op():
    ...
    In [11]: 4.5 Out[11]: 4.5

    In [12]: str(ss) Out[12]: 's'

    In [13]:

    In [14]: %hist -op >>> class b: ... pass ... >>> class s(b): ... def
    __str__(self): ... return 's' ... >>> >>> class r(b): ... def
    __repr__(self): ... return 'r' ... >>> class sr(s,r): pass >>> >>> bb=b()
    >>> ss=s() >>> rr=r() >>> ssrr=sr() >>> 4.5 4.5 >>> str(ss) 's' >>>
    """
...
def doctest_who():
    ...
    In [6]: %whos Variable Type Data/Info ---------------------------- alpha
    int 123 beta str beta

    In [7]: %who_ls Out[7]: ['alpha', 'beta']
    """
...
def doctest_precision():
    ...
    In [2]: %precision 5 Out[2]: '%.5f'

    In [3]: f.float_format Out[3]: '%.5f'

    In [4]: %precision %e Out[4]: '%e'

    In [5]: f(3.1415927) Out[5]: '3.141593e+00'
    """
...
def test_notebook_export_json():
    ...
    cmds = [u"a=1", u"def b():\n  return a**2", u"print('noÃ«l, Ã©tÃ©', b())"]
    for i, cmd in enumerate(cmds, start=1):
...
def test_file_unicode():
    ...
    with TemporaryDirectory() as td:
        ...
        with io.open(fname, encoding='utf-8') as f:
            s = f.read()
        nt.assert_in(u'linÃ©1\n', s)
        nt.assert_in(u'linÃ©2', s)
