...
def test_issue_114():
    """Check that multiline string literals don't expand as magic see
    http://github.com/ipython/ipython/issues/114
    """
...
def test_autocall_should_support_unicode():
    ip.magic('autocall 2')
    ip.user_ns['Ï€'] = lambda x: x
    try:
        nt.assert_equal(ip.prefilter('Ï€ 3'),'Ï€(3)')
    finally:
        ip.magic('autocall 0')
        del ip.user_ns['Ï€']
