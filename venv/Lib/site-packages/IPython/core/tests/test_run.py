...
def doctest_refbug():
    ...
    In [1]: _ip.clear_main_mod_cache() # random

    In [2]: %run refbug

    In [3]: call_f() lowercased: hello

    In [4]: %run refbug

    In [5]: call_f() lowercased: hello lowercased: hello
    """
...
def doctest_run_builtins():
    ...
    In [10]: t1 == t2 Out[10]: True

    In [10]: bid1 == bid2 Out[10]: True

    In [12]: try:
        ....: os.unlink(fname) ....: except: ....: pass ....:
    """
...
def doctest_run_option_parser():
    ...
    In [2]: %run print_argv.py print*.py ['print_argv.py']

    In [3]: %run -G print_argv.py print*.py ['print*.py']
    """
...
def doctest_run_option_parser_for_posix():
    ...
    In [1]: %run print_argv.py print\*.py ['print*.py']
...
def doctest_run_option_parser_for_windows():
    ...
    In [2]: %run print_argv.py 'print*.py' ['print*.py']
    """
...
def doctest_reset_del():
    ...
    In [2]: class A(object):
        ...: def __del__(self): ...: print(str("Hi")) ...:

    In [3]: a = A()

    In [4]: get_ipython().reset() Hi
...
class TestMagicRunPass(tt.TempFileMixin):
    ...
    def test_builtins_type(self):
        ...
        be a dict (it should be a module) by a previous use of %run. So we also
    ...
    def test_run_profile( self ):
        """Test that the option -p, which invokes the profiler, do not crash by
        invoking execfile
        """
        self.run_tmpfile_p()
...
"""
    ...
    def test_unicode(self):
        ...
        _ip.magic('run "%s"' % na)
        nt.assert_equal(_ip.user_ns['u'], u'ÐŽÑ‚â„–Ð¤')
