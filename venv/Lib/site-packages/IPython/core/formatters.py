    ...
    def format(self, obj, include=None, exclude=None):
        ...
        * image/svg+xml
        ...
        Args:
            ...
            include (list, tuple or set; optional): A list of format type
                strings (MIME types) to include in the format data dict. If this
                is set *only* the format types included in this list will be
                computed.
            exclude (list, tuple or set; optional): A list of format type string
                (MIME types) to exclude in the format data dict. If this is set
                all format types will be computed, except for those included in
                this argument. Mimetypes present in exclude will take precedence
                over the ones in include

        Returns:
            tuple of two dicts: **(format_dict, metadata_dict)** -- format_dict
            is a dictionary of key/value pairs, one of each format that was
            generated for the object. The keys are the format types, which will
            usually be MIME type strings and the values and JSON'able data
            structure containing the raw data for the representation in that
            format.

            metadata_dict is a dictionary of metadata about each mime-type
            output. Its keys will be a strict subset of the keys in format_dict.
...
class FormatterABC(metaclass=abc.ABCMeta):
    ...
    format data for a particular format type (MIME type). For example, an HTML
    formatter would have a format type of `text/html` and would return the HTML
    ...
    def __call__(self, obj):
        ...
        If the object cannot be formatted by this formatter, warn and return
        None.
...
class BaseFormatter(Configurable):
    ...
    extensible API for users to determine how their objects are formatted. The
    following logic is used to find a function to format an given object.

    1. The object is introspected to see if it has a method with the name
       :attr:`print_method`. If is does, that object is passed to that method
       for formatting.
    2. If no print method is found, three internal dictionaries are consulted to
       find print method: :attr:`singleton_printers`, :attr:`type_printers` and
       :attr:`deferred_printers`.

    Users should use these dictionaries to register functions that will be
    used to compute the format data for their objects (if those objects don't
    have the special print methods). The easiest way of using these dictionaries
    ...
    def lookup(self, obj):
        ...
        Returns:
            callable: **f** -- The registered formatting callable for the type.
    ...
    def for_type(self, typ, func=None):
        ...
        Args:
            typ (type or '__module__.__name__' string for a type): The class of
                the object that will be formatted using `func` .
            func (callable): A callable for computing the format data. `func`
                will be called with the object to be formatted, and will return
                the raw data in this formatter's format. Subclasses may use a
                different call signature for the `func` argument.

                If `func` is None or not specified, there will be no change,
                only returning the current value.

        Returns:
            callable: **oldfunc** -- The currently registered callable. If you
            are registering a new formatter, this will be the previous value (to
    ...
    def for_type_by_name(self, type_module, type_name, func=None):
        """Add a format function for a type specified by the full dotted module
        and name of the type, rather than the type of the object.

        Args:
            type_module (str): The full dotted name of the module the type is
                defined in, like ``numpy``.
            type_name (str): The name of the type (the class name), like
                ``dtype``
            func (callable): A callable for computing the format data. `func`
                will be called with the object to be formatted, and will return
                the raw data in this formatter's format. Subclasses may use a
                ...
                If `func` is None or unspecified, there will be no change, only
    ...
    def pop(self, typ, default=_raise_key_error):
        ...
        Args:
            typ (type or '__module__.__name__' string for a type):
            default (object): value to be returned if no formatter is registered
                for typ.

        Returns:
            object: **obj** -- The last registered object for the type.

        Raises:
            * KeyError if the type is not registered and default is not
              specified.
        """
    ...
    def _in_deferred_types(self, cls):
        ...
        Successful matches will be moved to the regular type registry for
        future use.
...
class PlainTextFormatter(BaseFormatter):
    ...
    def _float_precision_changed(self, change):
        ...
        float_precision can be set by int or str. This will set float_format,
        after interpreting input. If numpy has been imported, numpy print
        precision will also be set.
...
class HTMLFormatter(BaseFormatter):
    ...
    objects, define a :meth:`_repr_html_` method or use the :meth:`for_type` or
...
class MarkdownFormatter(BaseFormatter):
    ...
    or :meth:`for_type_by_name` methods to register functions that handle this.
...
class SVGFormatter(BaseFormatter):
    ...
    objects, define a :meth:`_repr_svg_` method or use the :meth:`for_type` or
    :meth:`for_type_by_name` methods to register functions that handle this.
...
class PNGFormatter(BaseFormatter):
    ...
    objects, define a :meth:`_repr_png_` method or use the :meth:`for_type` or
    :meth:`for_type_by_name` methods to register functions that handle this.
...
class JPEGFormatter(BaseFormatter):
    ...
    objects, define a :meth:`_repr_jpeg_` method or use the :meth:`for_type` or
    :meth:`for_type_by_name` methods to register functions that handle this.
...
class LatexFormatter(BaseFormatter):
    ...
    objects, define a :meth:`_repr_latex_` method or use the :meth:`for_type` or
    :meth:`for_type_by_name` methods to register functions that handle this.

    The return value of this formatter should be a valid LaTeX equation,
    enclosed in either ```$```, ```$$``` or another LaTeX equation environment.
    """
...
class JSONFormatter(BaseFormatter):
    ...
    objects, define a :meth:`_repr_json_` method or use the :meth:`for_type` or
    :meth:`for_type_by_name` methods to register functions that handle this.
...
class PDFFormatter(BaseFormatter):
    ...
    objects, define a :meth:`_repr_pdf_` method or use the :meth:`for_type` or
    :meth:`for_type_by_name` methods to register functions that handle this.
...
class IPythonDisplayFormatter(BaseFormatter):
    """An escape-hatch Formatter for objects that know how to display
    themselves.
    ...
    :meth:`for_type_by_name` methods to register functions that handle this.
    Unlike mime-type displays, this method should not return anything, instead
    calling any appropriate display methods itself.
    ...
    IPython 6.1 introduces `_repr_mimebundle_` for displaying custom
    mime-types, so `_ipython_display_` should only be used for objects that
...
class MimeBundleFormatter(BaseFormatter):
    ...
    def __call__(self, obj, include=None, exclude=None):
        ...
        Identical to parent's method but we pass extra parameters to the
        method.

        Unlike other _repr_*_ `_repr_mimebundle_` should allow extra kwargs,
        in particular `include` and `exclude` .
...
def format_display_data(obj, include=None, exclude=None):
    ...
    Returns:
        ...
        * **include** (*list or tuple, optional*) -- A list of format type
          strings (MIME types) to include in the format data dict. If this is
          set *only* the format types included in this list will be computed.
        * **exclude** (*list or tuple, optional*) -- A list of format type
          string (MIME types) to exclude in the format data dict. If this is set
          all format types will be computed, except for those included in this
          argument.
    """
