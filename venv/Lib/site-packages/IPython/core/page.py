...
def _detect_screen_size(screen_lines_def):
    ...
    test suite), so it's separated out so it can easily be called in a try
    block.
...
def pager_page(strng, start=0, screen_lines=0, pager_cmd=None):
    ...
    If you set screen_lines to a number <=0, page() will try to
    auto-determine your screen size and will only use up to
    (screen_size+screen_lines) for printing, paging after that. That is, if you
    want auto-detection but need to reserve the bottom 3 lines of the screen,
    use screen_lines = -3, and for auto-detection without any lines reserved
    simply use screen_lines = 0.
...
def page(data, start=0, screen_lines=0, pager_cmd=None):
    """Display content in a pager, piping through a pager after a certain
    length.
