...
if sqlite3 is not None:
    ...
    class DatabaseError(Exception):
        """Dummy exception when sqlite could not be imported. Should never
        occur.
        """
...
class HistoryAccessor(HistoryAccessorBase):
    ...
    def __init__(self, profile='default', hist_file=u'', **traits):
        ...
        Args:
            ...
            hist_file (str): Path to an SQLite history database stored by
                IPython. If specified, hist_file overrides profile.
            **traits:
        """
    ...
    def _get_hist_file_name(self, profile='default'):
        ...
        Args:
            profile (str): The name of a profile which has a history file.
        """
    ...
    def _run_sql(self, sql, params, raw=True, output=False):
        ...
        Args:
            sql (str): Any filtering expressions to go after SELECT ... FROM ...
            params (tuple): Parameters passed to the SQL query (to replace "?")
            raw:
            output (raw,): See :meth:`get_range`
    ...
    def get_session_info(self, session):
        ...
        Returns:
            ...
            * **end** (*datetime*) -- Timestamp for the end of the session, or
              None if IPython crashed.
    ...
    def get_tail(self, n=10, raw=True, output=False, include_latest=False):
        ...
        Args:
            ...
            raw:
            output (raw,): See :meth:`get_range`
            include_latest (bool): If False (default), n+1 lines are fetched,
                and the latest one is discarded. This is intended to be used
                where the function is called by a user command, which it should
                not return.
    ...
    def search(self, pattern="*", raw=True, search_raw=True,
        ...
        Args:
            ...
            raw:
            search_raw (bool): If True, search the raw input, otherwise, the
                parsed input
            output (raw,): See :meth:`get_range`
            n (None or int): If an integer is given, it defines the limit of
                returned entries.
    ...
    def get_range(self, session, start=1, stop=None, raw=True,output=False):
        ...
        Args:
            ...
            stop (int): End of line range (excluded from output itself). If
                None, retrieve to the end of the session.
            raw (bool): If True, return untranslated input
            output (bool): If True, attempt to include output. This will be
                'real' Python objects for the current session, or text reprs
                from previous sessions if db_log_output was enabled at the time.
                Where no output is found, None is used.

        Returns:
            entries: An iterator over the desired lines. Each line is a 3-tuple,
            either (session, line, input) if output is False, or (session, line,
            (input, output)) if output is True.
        """
        if stop:
...
class HistoryManager(HistoryAccessor):
    ...
    def _get_hist_file_name(self, profile=None):
        ...
        The profile parameter is ignored, but must exist for compatibility
    ...
    def end_session(self):
        """Close the database session, filling in the end time and line
        count.
        """
        self.writeout_cache()
        with self.db:
    ...
    def get_session_info(self, session=0):
        ...
        Args:
            session (int): Session number to retrieve. The current session is 0,
                and negative numbers count back from current session, so -1 is
    ...
    def store_inputs(self, line_num, source, source_raw=None):
        ...
        Args:
            ...
            source_raw (str, optional): If given, this is the raw input without
                any IPython transformations applied to it. If not given,
                ``source`` is used.
        """
    ...
    def store_output(self, line_num):
        """If database output logging is enabled, this saves all the outputs
        from the indicated prompt number to the database. It's called by
        run_cell after code has been executed.
