...
class InputSplitter(object):
    ...
    :meth:`push`. It will return on each push whether the currently pushed code
    could be executed already. In addition, it provides a method called
    :meth:`push_accepts_more` that can be used to query whether more input can
    be pushed into a single interactive block.
    ...
    def push(self, lines:str) -> bool:
        ...
        Args:
            lines (str): One or more lines of Python input.

        Returns:
            boolean: **is_complete** -- True if the current input source (the
            result of the current input plus prior inputs) forms a complete
            Python execution block. Note that this value is also stored as a
            private attribute (``_is_complete``), so it can be queried at any
            time.
        """
    ...
    def push_accepts_more(self):
        ...
        This method is meant to be used by line-oriented frontends, who need
        to guess whether a block is complete or not based solely on prior and
...
class IPythonInputSplitter(InputSplitter):
    ...
    def _transform_line(self, line):
        ...
        Returns any output from the transformers, or None if a transformer is
        accumulating lines.
