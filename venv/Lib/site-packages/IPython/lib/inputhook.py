...
class InputHookManager(object):
    ...
    def clear_inputhook(self, app=None):
        ...
        Args:
            app (optional, ignored): This parameter is allowed only so that
                clear_inputhook() can be called with a similar interface as all
                the ``enable_*`` methods. But the actual value of the parameter
                is ignored. This uniform interface makes it easier to have
                user-level entry points in the main IPython app like
                :meth:`enable_gui`.
        """
    ...
    def clear_app_refs(self, gui=None):
        ...
        can happen if a user doesn't hold a reference themselves. This method is
        provided to clear the references we are holding.

        Args:
            gui (None or str): If None, clear all app references. If ('wx',
                'qt4') clear the app for that toolkit. References are not held
                for gtk or tk as those toolkits don't have the notion of an app.
        """
    ...
    def enable_gui(self, gui=None, app=None):
        ...
        This is a higher level method than :meth:`set_inputhook` - it uses
        the GUI name to look up a registered object which enables the input hook
        for that GUI.

        Args:
            gui (optional, string or None): If None (or 'none'), clears input
                hook, otherwise it must be one of the recognized GUI names (see
                ``GUI_*`` constants in module).
            app (optional, existing application object.): For toolkits that have
                the concept of a global app, you can supply an existing one. If
                not given, the toolkit will be probed for one, and if none is
                found, a new one will be created. Note that GTK does not have
                this concept, and passing an app if ``gui=="GTK"`` will raise an
                error.
...
class WxInputHook(InputHookBase):
    def enable(self, app=None):
        ...
        Args:
            app (WX Application, optional.): Running application to use. If not
                given, we probe WX for an existing application object, and
        ...
        Notes:
            ...
            If ``app`` is not given we probe for an existing one, and return it
            if found. If no existing app is found, we create an :class:`wx.App`
            as follows::
...
class Qt4InputHook(InputHookBase):
    def enable(self, app=None):
        ...
        Args:
            app (Qt Application, optional.): Running application to use. If not
                given, we probe Qt for an existing application object, and
                create a new one if none is found.

        Notes:
            This methods sets the PyOS_InputHook for PyQt4, which allows the
            PyQt4 to integrate with terminal based applications like IPython.

            If ``app`` is not given we probe for an existing one, and return it
            if found. If no existing app is found, we create an
            :class:`QApplication` as follows::
...
class GtkInputHook(InputHookBase):
    def enable(self, app=None):
        ...
        Args:
            app (ignored): Ignored, it's only a placeholder to keep the call
                signature of all gui activation methods consistent, which
                simplifies the logic of supporting magics.

        Notes:
            This methods sets the PyOS_InputHook for PyGTK, which allows the
            PyGTK to integrate with terminal based applications like IPython.
        """
        warn("This function is deprecated since IPython 5.0 and will be removed in future versions.",
                DeprecationWarning, stacklevel=2)
        import gtk
...
class TkInputHook(InputHookBase):
    def enable(self, app=None):
        ...
        Args:
            app (toplevel Tkinter.Tk widget, optional.): Running toplevel widget
                to use. If not given, we probe Tk for an existing one, and
                create a new one if none is found.
...
class GlutInputHook(InputHookBase):
    def enable(self, app=None):
        ...
        Notes:
            This methods sets the PyOS_InputHook for GLUT, which allows the GLUT
            to integrate with terminal based applications like IPython. Due to
            GLUT limitations, it is currently not possible to start the event
            loop without first creating a window. You should thus not create
            another window but use instead the created one. See 'gui-glut.py' in
            the docs/examples/lib directory.
    ...
    def disable(self):
        ...
        dummy one and set the timer to a dummy timer that will be triggered very
        far in the future.
        """
        warn("This function is deprecated since IPython 5.0 and will be removed in future versions.",
                DeprecationWarning, stacklevel=2)
...
class PygletInputHook(InputHookBase):
    def enable(self, app=None):
        ...
        Notes:
            ...
            pyglet to integrate with terminal based applications like IPython.
        """
        warn("This function is deprecated since IPython 5.0 and will be removed in future versions.",
                DeprecationWarning, stacklevel=2)
...
class Gtk3InputHook(InputHookBase):
    def enable(self, app=None):
        ...
        Notes:
            This methods sets the PyOS_InputHook for Gtk3, which allows the Gtk3
