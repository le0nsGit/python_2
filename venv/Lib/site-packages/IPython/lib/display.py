...
class Audio(DisplayObject):
    ...
    function, it will result in Audio controls being displayed in the frontend
    (only works in the notebook).
    ...
    Args:
        ...
        embed (boolean): Should the audio data be embedded using a data URI
            (True) or should the original source be referenced. Set this to True
            if you want the audio to playable later with no internet connection
            in the notebook.

            Default is `True` , unless the keyword argument `url` is set, then
            default value is `False` .
        rate (integer): The sampling rate of the raw data. Only required when
            data parameter is being used as an array
        autoplay (bool): Set to True if the audio should immediately start
            playing. Default is `False` .
        normalize (bool): Whether audio should be normalized (rescaled) to the
            maximum possible range. Default is `True` . When set to `False` ,
            `data` must be between -1 and 1 (inclusive), otherwise an error is
            raised. Applies only when `data` is a list or array of samples;
            other types of audio are never normalized.
...
class FileLink(object):
    """Class for embedding a local file link in an IPython session, based on
    path
    ...
    def __init__(self,
        ...
        Args:
            ...
            url_prefix (str): prefix to be prepended to all files to form a
                working link [default: '']
            result_html_prefix (str): text to append to beginning to link
                [default: '']
            result_html_suffix (str): text to append at the end of link
                [default: '<br>']
        """
        if isdir(path):
...
class FileLinks(FileLink):
    ...
    def __init__(self,
        ...
        included_suffixes  : list
            Filename suffixes to include when formatting output [default:
            include all files]
        ...
        included_suffixes  : list
            The file suffixes that should be included in the output (passing
            None meansto include all suffixes in the output in the built-in
            formatters)

        recursive  : boolean
            Whether to recurse into subdirectories. Default is True.

        The function should return a list of lines that will be printed in
        the notebook (if passing notebook_display_formatter) or the terminal (if
    ...
    def _get_display_formatter(self,
        ...
        dirname_output_format: string to use for formatting directory
            names, dirname will be substituted for a single "%s" which must
            appear in this string

        fname_output_format: string to use for formatting file names,
            if a single "%s" appears in the string, fname will be substituted if
            two "%s" appear in the string, the path to fname will be
...
class Code(TextDisplayObject):
    ...
    Args:
        ...
        language (str): The short name of a Pygments lexer to use for
            highlighting. If not specified, it will guess the lexer based on the
            filename or the code. Available lexers:
