...
default space.  `p.pretty` prettyprints another object using the pretty print
    ...
    Args:
        python3 (bool): If `True` , then build an IPython lexer from a Python 3
            lexer.
    """
...
def _safe_getattr(obj, attr, default=None):
    ...
    Same as getattr, but will return ``default`` on any Exception, rather
    than raising.
...
def _sorted_for_pprint(items):
    """Sort the given items for pretty printing. Since some predictable sorting
    is better than no sorting at all, we sort on the string representation if
...
class PrettyPrinter(_PrettyPrinterBase):
    ...
    this printer knows nothing about the default pprinters or the
    ...
    def begin_group(self, indent=0, open=''):
        """Begin a group. The first parameter specifies the indentation for the
        next line (usually the width of the opening text), the second the
        opening text. All parameters are optional.
...
class RepresentationPrinter(PrettyPrinter):
    ...
    class in a threaded environment. Always lock it or reinstanciate it.

    Instances also have a verbose flag callbacks can access to control their
    output. For example the default instance repr prints all attributes and
    methods that are not prefixed by an underscore if the printer is in verbose
    mode.
    """
...
def _seq_pprinter_factory(start, end):
    """Factory that returns a pprint function useful for sequences. Used by the
    default pprint for tuples, dicts, and lists.
