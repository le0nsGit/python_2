...
def install_editor(template, wait=False):
    ...
    Args:
        template (basestring): run_template acts as a template for how your
            editor is invoked by the shell. It should contain '{filename}',
            which will be replaced on invocation with the file name, and
            '{line}', $line by line number (or 0) to invoke the file with.
        wait (bool): If `wait` is true, wait until the user presses enter before
            returning, to facilitate non-blocking editors that exit immediately
            after the call.
    """
...
def idle(exe=u'idle'):
    ...
    Args:
        exe (str, None): If none, should be pretty smart about finding the
            executable.
    """
    if exe is None:
        import idlelib
