...
def skipif(skip_condition, msg=None):
    ...
    Args:
        skip_condition (bool or callable): Flag to determine whether to skip
            test. If the condition is a callable, it is used at runtime to
            dynamically make the decision. This is useful for tests that may
            require costly imports, to delay the cost until the test suite is
            actually executed.
        msg (string): Message to give on raising a SkipTest exception.

    Returns:
        function: **decorator** -- Decorator, which, when applied to a function,
        causes SkipTest to be raised when the skip_condition was True, and the
        function to be called normally otherwise.

    Notes:
        You will see from the code that we had to further decorate the decorator
        with the nose.tools.make_decorator function in order to transmit
...
def skip(msg=None):
    ...
    Returns:
        function: **decorator** -- Decorator, which, when applied to a function,
        causes SkipTest to be raised, with the optional message added.
    """
