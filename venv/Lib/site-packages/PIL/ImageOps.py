...
def autocontrast(image, cutoff=0, ignore=None, mask=None):
    ...
    lightest and darkest pixels from the histogram, and remaps the image so that
    the darkest pixel becomes black (0), and the lightest becomes white (255).

    Args:
        image: The image to process.
        cutoff: The percent to cut off from the histogram on the low and high
            ends. Either a tuple of (low, high), or a single number for both.
        ignore: The background pixel value (use None for no background).
        mask: Histogram used in contrast operation is computed using pixels
            within the mask. If no mask is given the entire image is used for
...
def colorize(image, black, white, mid=None, blackpoint=0, whitepoint=255, midpoint=127):
    ...
    maps all black pixels in the source image to the first color and all white
    pixels to the second color. If ``mid`` is specified, it uses three-color
    mapping. The ``black`` and ``white`` arguments should be RGB tuples or color
    names; optionally you can use three-color mapping by also specifying
    ``mid``. Mapping positions for any of the colors can be specified (e.g.
    ``blackpoint``), where these parameters are the integer value corresponding
    to where the corresponding color should be mapped. These parameters must
    have logical order, such that ``blackpoint <= midpoint <= whitepoint`` (if
    ``mid`` is specified).
...
def pad(image, size, method=Image.BICUBIC, color=None, centering=(0.5, 0.5)):
    ...
    Args:
        ...
        size: The requested output size in pixels, given as a (width, height)
            tuple.
        method: What resampling method to use. Default is
            :py:attr:`PIL.Image.BICUBIC`. See :ref:`concept-filters`.
        color: The background color of the padded image.
        centering: Control the position of the original image within the padded
            version.

                (0.5, 0.5) will keep the image centered (0, 0) will keep the
                image aligned to the top left (1, 1) will keep the image aligned
                to the bottom right
...
def scale(image, factor, resample=Image.BICUBIC):
    ...
    Args:
        ...
        resample: What resampling method to use. Default is
            :py:attr:`PIL.Image.BICUBIC`. See :ref:`concept-filters`.
...
def deform(image, deformer, resample=Image.BILINEAR):
    ...
    Args:
        ...
        deformer: A deformer object. Any object that implements a ``getmesh``
            method can be used.
        resample: An optional resampling filter. Same values possible as in the
...
def equalize(image, mask=None):
    ...
    Args:
        ...
        mask: An optional mask. If given, only the pixels selected by the mask
...
def fit(image, size, method=Image.BICUBIC, bleed=0.0, centering=(0.5, 0.5)):
    ...
    Args:
        ...
        method: What resampling method to use. Default is
            :py:attr:`PIL.Image.BICUBIC`. See :ref:`concept-filters`.
        bleed: Remove a border around the outside of the image from all four
            edges. The value is a decimal percentage (use 0.01 for one percent).
            The default value is 0 (no border). Cannot be greater than or equal
            to 0.5.
        centering: Control the cropping position. Use (0.5, 0.5) for center
            cropping (e.g. if cropping the width, take 50% off of the left side,
            and therefore 50% off the right side). (0.0, 0.0) will crop from the
            top left corner (i.e. if cropping the width, take all of the crop
            off of the right side, and if cropping the height, take all of it
            off the bottom). (1.0, 0.0) will crop from the bottom left corner,
            etc. (i.e. if cropping the width, take all of the crop off the left
            side, and if cropping the height take none from the top, and
            therefore all off the bottom).
