...
def connect(*args, **kwargs): # --> a db-api connection object
    ...
    call using: :connection_string -- An ADODB formatted connection string,
    see:
...
class Connection(object):
    ...
    def _closeAdoConnection(self):                  #all v2.1 Rose
        """close the underlying ADO Connection object, rolling it back first if
    ...
    def commit(self):
        ...
        on. Database modules that do not support transactions should implement
    ...
    def _rollback(self):
        ...
        have Python generate an AttributeError in case the method is requested.
        This allows the programmer to check for database capabilities using the
        standard hasattr() function.

        For some dynamically configured interfaces it may not be appropriate
        to require dynamically making the method available. These interfaces
        should then raise a NotSupportedError to indicate the non-ability to
        perform the roll back when the method is invoked.
        """
        self.messages=[]
    ...
    def _suggest_error_class(self):
        """Introspect the current ADO Errors and determine an appropriate error
        class.

        Error.SQLState is a SQL-defined error condition, per the SQL
        specification: http://www.contrib.andrew.cmu.edu/~shadow/sql/sql1992.txt

        The 23000 class of errors are integrity errors. Error 40002 is a
        transactional integrity error.
        """
...
##   allows the programmer to override the connection's default error handler
    ...
    def get_returned_parameters(self):
        """with some providers, returned parameters and the .return_value are
        not available until after the last recordset has been read. In that
        case, you must coll nextset() until it returns None, then call this
        method to get your returned information.
        """
    ...
    def callproc(self, procname, parameters=None):
        ...
        parameters must contain one entry for each argument that the sproc
        expects. The result of the call is returned as modified copy of the
        input sequence. Input parameters are left untouched, output and
        input/output parameters replaced with possibly new values.

        The sproc may also provide a result set as output, which is available
        through the standard .fetch*() methods. Extension: A "return_value"
        property may be set on the cursor if the sproc defines an integer return
        value.
    ...
    def execute(self, operation, parameters=None):
        ...
        details). [5] A reference to the operation will be retained by the
        cursor. If the same operation object is passed in again, then the cursor
        can optimize its behavior. This is most effective for algorithms where
        ...
        as input. The term "bound" refers to the process of binding an input
    ...
    def _fetch(self, limit=None):
        ...
        limit -- Number of rows to fetch, or None (default) to fetch all
        rows.
    ...
    def fetchall(self):
        ...
        of this operation. An Error (or subclass) exception is raised if the
        previous call to executeXXX() did not produce any result set or no call
        was issued yet.
        """
        self.messages=[]                
        return self._fetch()
    ...
    def nextset(self):
        """Skip to the next available recordset, discarding any remaining rows
        from the current recordset.
