...
def fix_uri(uri, kwargs):
    """convert a generic pyro uri with '0.0.0.0' into the address we actually
    called
...
def fixpickle(x):
    """pickle barfs on buffer(x) so we pass as array.array(x) then restore to
    original form for .execute()
